...
class Buyer
{
	...
	public function show_product_view($name,$Manufacturer,$CATEGORY,$Price,$No_of_product,$Technical_detail,$Pic_1,$Pic_2,$Pic_3,$id)
	{
		...
		<div class="row-fluid">
			...
			<div class="span12 row-fluid" >
					...
					<div class="carousel slide carousel-fade" id="myCarousel">
			            <div class="carousel-inner">
			              ...
			              foreach ($aa as $key => $value) {
			                ...
			                echo '">';
			                echo '<img alt="" style="height :500px;width: 300px;" src="data:image/jpeg;base64,'.base64_encode($key).'" />">';
				...
				<div class="span3">
					<legend>Related advertisement</legend>
					<?php
					$this->show_add($name);
					?>
				</div>
	...
	public function show_add($v)
	{
		$v='%'.$v.'%';
		require_once "database_config.inc.php";
	    $conn = oci_connect(db_user, db_pass,db_service);
	    if($conn) {
	      $q = 'SELECT name,description,image from advertisement where tags like :vvv';
	      $query = oci_parse($conn, $q);
	      oci_bind_by_name($query, ':vvv',$v);
	      oci_execute($query);
	      $x=0;
	      while($db_data=oci_fetch_array($query)){
	      	...
	      	$desc=$db_data[1];
	      	$Pic=$db_data[2]->load();
	      	echo '<a href="#"><div class="span12">'.$name.'</div>';
	      	echo '<div class="span12"><img style="height :100px;width: 100px;"src="data:image/jpeg;base64,'.base64_encode($Pic).'" alt=""/></div></a>';
	      	echo '<div class="span12">'.$desc.'</div>';
	      	$x++;
	      	if($x>2)break;
	      }
	      oci_close($conn);
