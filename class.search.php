...
class Search
{
	...
	public function search($page=0)
	{
		...
		$rr='';
		if($this->keyword!=='%%%')$rr=substr($this->keyword,1, -1);
		echo '<legend>Searched for '.$rr.'</legend>';
	    ...
	    if($conn) {
	      ...
	      if(isset($this->product_type))$q.='and category=:aproduct_type ';
	      ...
	      if(isset($this->product_type))oci_bind_by_name($query, ':aproduct_type', $this->product_type);
	      ...
	      while($pp<=$x&&$db_data=oci_fetch_array($query)){
	        ...
	        $price=$db_data[1];
	        $lob=$db_data[2]->load();
	        echo '<td><img style="height :300px;width: 180px;"src="data:image/jpeg;base64,'.base64_encode($lob).'" alt=""/>';
