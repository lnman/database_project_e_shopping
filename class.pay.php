...
class Pay
{
	...
	public static function all_seller($val,$ad)
	{
		...
		if($conn) {
			...
			foreach ($val as $key => $value) {
				...
				$q = "select id,userid,price,no_of_product from product where id=:ax";
				...
				if($db_data)
				{
					...
					$tttt->update_product($key,"","","","",$db_data[3]-$value,"","","","");
			...
			$xx=new notific_booked_bookmark();
			foreach ($desc as $key => $value) {
				...
				$q = "insert into ordered values (:aid,:aid1,:aid2,:adesc,:aprice,:address,sysdate)";
				$query = oci_parse($conn, $q);
				...
				oci_bind_by_name($query, ':address', $ad);
				oci_execute($query, OCI_DEFAULT) or die ("Unable to execute query");
				$xx->add_notification($key,$user.'ordered for '.$value.'.send within 5 days at'.$ad);
